...
<div class='flex flex-col space-y-3  mt-10 '>
    ...
    <input bind:value={email} class=" text-center input w-1/4 m-auto" title="Input (email)" type="email" placeholder="kate@example.com" autocomplete="email" on:keydown={async (e) => {handleSubmission(e)}} />    
</div>
        ...
        {#each $authStore.rcvdReqs as req}
            ...
            <div class='card p-4 w-72 shadow-xl flex flex-col items-center justify-center'>
                ...
                <div class='mt-3 flex gap-4'>
                    <!-- Accept -->
                    <button class='btn btn-md variant-filled-success' on:click = {() => dataHandlers.acceptReq(req)}>Accept</button>
                    <!-- Deny -->
